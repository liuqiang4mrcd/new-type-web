...
6. **单 Section 验证**：`pnpm validate-section --campaign <campaign-name> <SectionName>`，16 项检查全部通过
7. **更新进度**：`.feedback/progress.md` 标记为 `validated`，并在对话中报告：`<SectionName> 单组件校验通过：16/16`
...
- ✅ **自动验证**：每 Section 完成后执行 `pnpm validate-section --campaign <campaign-name> <SectionName>`，全部完成后执行 `pnpm validate-section --campaign <campaign-name> --all`
- ✅ **Code Review**：开发者审查代码
