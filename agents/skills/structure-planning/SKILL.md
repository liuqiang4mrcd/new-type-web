...
description: H5 活动页结构规划能力模块。用于 designer agent 的第 2 步，产出 structure、Layout Spec、Interaction Spec、Effect Spec、Image Asset Inventory、Uncertainty List 和状态适配分析。
---

# Structure Planning Skill

用于结构规划。为降低上下文成本，本模块按子阶段读取规则，禁止在未需求确认的情况下执行结构规划。
...
- 本文件全文
- `agents/shared/STRUCTURE_OUTPUT.md` — Section 拆分决策树和输出模板
- `agents/shared/DESIGN_INPUT.md` 中与冲突处理相关的章节
- 若素材或需求中存在图片类元素，读取 `agents/shared/DESIGN_OUTPUT.md` 的 `Image Asset Inventory Schema`，按该 schema 输出图片资产清单。
...
**⚠️ 本阶段的 Section 拆分决策必须遵守 `agents/shared/STRUCTURE_OUTPUT.md` 的决策树（节点 1-4）和输出模板。每个 Section 的 4 个枚举字段（数据来源、业务闭环、Tab归属、弹窗复杂度）必须填写。**
...
清单 schema、字段枚举、分类规则和实现门禁以 `agents/shared/DESIGN_OUTPUT.md` 的 `Image Asset Inventory Schema` 与 `Image Asset Gate` 为准。`structure-planning` 只负责在结构规划阶段识别图片元素并输出清单，不复制或改写实现规则。
...
图片类关键元素必须同时能追溯到 `Image Asset Inventory`。头像、礼物、奖品、道具、活动主图、榜单头像、奖励卡图片区域不能只写在 Layout Spec 而不进入图片资产清单。

### Interaction Spec

这是交互链路唯一真源。代码中的 `defaultActions`、`preview-state`、`stateTransitions` 和 Runtime actions 命名必须与它对齐。
...
| 级别 | 定义 | 处理方式 |
|------|------|---------|
| `blocking` | 影响 Section 边界、核心交互链路、数据字段语义 | 必须向用户确认，不得作为假设进入下一阶段 |
| `assumption` | 影响具体数值但不影响结构/交互链路 | 记录假设值并按行业惯例取默认值；用户可在审批时修正 |
| `no-impact` | 纯装饰微调 | 不进入 Uncertainty List，在实现阶段按视觉方案自由发挥 |
...
- Section 边界或关键元素归属不清楚 → blocking
- 关键元素顺序、父级、间距不清楚 → 按情况：影响结构为 blocking，仅数值为 assumption
- fixed / sticky / scroll / overlay 行为不清楚 → blocking
- 弹窗触发、关闭、目标状态不清楚 → blocking
- 多状态差异不清楚 → blocking
- 关键元素是否必须保留不清楚 → blocking

所有 blocking 级不确定项必须在第 3.5 步设计方案审批前解决；assumption 级默认 accepted，用户可修正。
