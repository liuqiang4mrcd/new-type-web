...
用于需求收集、素材分析和结构规划。为降低上下文成本，本模块按子阶段读取规则，禁止在需求收集阶段预读结构规划和模板实施文档。
...
- 需要判断模板目录、Playground 或 Runtime 注册方式时，读取 `docs/campaign-template.md` 的相关章节；结构规划阶段不默认全文预读。

## 第 1 步：需求收集

必须明确并写入当前 feedback 工作区的 `demand.md`。本阶段不输出 Section 拆分、Layout Spec、Interaction Spec 或 Effect Spec：
...
- 当前 feedback 工作区下的 `demand.md`、`structure.md`、`progress.md` 和对用户输出的需求/结构方案必须使用中文。
...
清单 schema、字段枚举、分类规则和实现门禁以 `agents/shared/DESIGN_OUTPUT.md` 的 `Image Asset Inventory Schema` 与 `Image Asset Gate` 为准。`design-input` 只负责在结构规划阶段识别图片元素并输出清单，不复制或改写实现规则。
