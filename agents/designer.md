...
2. **判断是否恢复任务**：若用户在继续已有活动、上下文压缩后恢复、或当前阶段不确定，先读取 `apps/<campaign-name>/.feedback/status.json`；若缺失或与文件系统冲突，再读取 `progress.md` 并运行 `pnpm audit-feedback --campaign <campaign-name>` 校正。旧项目缺少 `status.json` 时，运行 `pnpm audit-feedback --campaign <campaign-name> --write-status` 生成迁移状态。
3. **判断新项目 vs 修改**：
...
恢复时若 `status.json` 与实际文件或验证日志冲突，先审计并修正 `status.json`，再继续当前阶段。旧项目缺少 `status.json` 时，使用 `pnpm audit-feedback --campaign <campaign-name> --write-status` 从 `progress.md` 和文件系统生成一次迁移记录。
