...
| 开发写法 | 设计稿 px 直接写入样式代码，构建时 `postcss-pxtorem` 自动转为 rem（`rootValue: 100`，即 100px→1rem） |
| 页面底部留白 | 90px |
| 布局方式 | 纵向滚动，flex-col |
...
1. **设计稿单位换算**：所有数值基于 750px 设计稿。**代码中直接写设计稿 px 值，不手动除以 2**，`postcss-pxtorem`（`rootValue: 100`）构建时自动转换为 rem，在不同屏幕上等比缩放。（等效规律：设计稿 200px → 2rem → 375 屏上实际渲染 ≈100px）
